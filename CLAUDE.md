...
- **Deal hunter / marketplace**: Facebook Marketplace + Craigslist listing harvest → LLM triage → comps → deep evaluation, orchestrated by Temporal workflows (`backend/src/temporal/`, `backend/src/marketplace/`). The Facebook side runs on a separate "browser box" worker (`agent/`) that drives a logged-in Chrome over CDP on its own Temporal task queue. Deal alerts are **push notifications to the Android app**, not browser notifications: whichever process inserts an `mp_notifications` row (worker activity or API) calls `push/deals.ts`, which honors the user's `notification_prefs` (`enabled` master switch plus per-event `events.{deals,sold,loginNeeded}` switches; the pure gates live in `marketplace/notificationRules.ts`) and sends a data-only FCM message (`push/payload.ts` is the pure, tested payload builder) to every token in `push_devices`. Phones register via `POST /me/devices` on each sync (`android-app/.../push/`); a tap opens the deal in the web app (`WEB_APP_URL`, falling back to the first CORS origin). Sending requires real Firebase service-account credentials in the process (`GOOGLE_APPLICATION_CREDENTIALS[_JSON]`).
...
- **Thrawn**: fantasy football analyzer over Sleeper data (`backend/src/thrawn/`, `frontend/src/thrawn/`). Valuation is Points Above Replacement per game with a simulated fringe-bench replacement baseline — read the header comment in `backend/src/thrawn/engine.ts`. The Live tab (`GET /thrawn/leagues/:id/live`) has no push source: the service polls Sleeper's public matchups, weekly stats, and schedule endpoints behind a short TTL cache and the pure `thrawn/live.ts` joins them into a scoreboard; the web client re-fetches every 30s while any NFL game is in progress.
- **Moneyball**: ultimate frisbee player ratings (`backend/src/moneyball/`, `frontend/src/moneyball/`). Shared roster (committed `roster.ts`, upserted on boot, produced by the `import.ts` scraper), per-user 1-10 ratings on 12 stats, averaged into category scores and a weighted OVR via the pure `engine.ts`. See `docs/MONEYBALL.md`.
...
- **Auth**: every route sits behind `auth/middleware.ts`, which verifies Firebase Bearer ID tokens, looks the email up in `user_access` (or `BOOTSTRAP_ADMIN_EMAILS`), lazy-creates users (seeding their category/activity list on first sign-in), then authorizes by feature. Features are flat yes/no per product; `auth/access.ts` (pure, tested) holds the catalog and the URL-prefix → feature map — **every new route family must be added there or it fails closed (403)**. Admins manage accounts from the web Admin tab (`routes/admin.ts`, `frontend/src/admin/`). `GET /me` returns `isAdmin` + `features` so clients know what to render.
